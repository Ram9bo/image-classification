...
# Load the CSV file
history_df = pd.read_csv('history-Standard.csv')
...
all_preds = np.load('all-preds-Standard.npy').reshape((50, 120))
all_labels = np.load('all-labels-Standard.npy').reshape((50, 120))
...
plt.ylabel('True')
plt.savefig('normalized_confusion_matrix_with_std.png')
plt.close()